<?php
// =========================================================
// hks/edit.php — HKS Bildirimi Düzenle (sadece taslak)
// =========================================================
...
if ($n['status'] !== 'draft') {
    set_flash('error', 'Sadece taslak durumdaki bildirimler düzenlenebilir.');
    header('Location: view.php?id=' . $id);
    exit;
}

$errors = [];
$old    = $n; // varsayılan: mevcut veriler
    ...
    if (empty($errors)) {
        ...
        if ($action === 'send') {
            ...
            if ($result['ok']) {
                ...
                set_flash('error', 'Kaydedildi. Gönderim: ' . ($result['message'] ?? ''));
            }
        } else {
...
$notification_types = [
    'Satış Bildirimi', 'Alış Bildirimi', 'Transfer Bildirimi', 'İade Bildirimi',
];
$package_types = ['Kasa', 'Palet', 'Çuval', 'Koltu', 'Bidon', 'Diğer'];

render_header('HKS Bildirim Düzenle #' . $id);
render_flash();
?>

<div class="page-head">
    <div>
        <h1>🏛 Bildirim Düzenle #<?= (int)$n['id'] ?></h1>
    </div>
    <div class="page-head-actions">
        <a href="view.php?id=<?= (int)$id ?>" class="btn btn-ghost">← Geri</a>
    </div>
</div>
...
<form method="post" action="edit.php?id=<?= (int)$id ?>" autocomplete="off">
    ...
    <div class="form-section">
        ...
        <div class="form-group">
            ...
            <input type="text" id="product_name" name="product_name"
                   value="<?= hks_h($old['product_name'] ?? '') ?>" required>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="quantity" name="quantity" step="0.001" min="0.001"
                       value="<?= hks_h((string)($old['quantity'] ?? '')) ?>" required>
            </div>
            <div class="form-group">
    ...
    <div class="form-section">
        ...
        <div class="form-group">
            ...
            <input type="date" id="shipment_date" name="shipment_date"
                   value="<?= hks_h($old['shipment_date'] ?? '') ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="note" name="note" rows="3"><?= hks_h($old['note'] ?? '') ?></textarea>
        </div>
    </div>

    <div class="form-actions">
        <a href="view.php?id=<?= (int)$id ?>" class="btn btn-ghost btn-lg">İptal</a>
