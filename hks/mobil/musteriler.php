...
<div class="mob-contact-card">
    <div class="mob-contact-name"><?= hks_mob_h($c['display_name']) ?></div>
    <div class="mob-contact-meta">
        <span><?= hks_mob_h($c['tc_vkn']) ?></span>
        <?php if (!empty($c['phone'])): ?><span>· <?= hks_mob_h($c['phone']) ?></span><?php endif; ?>
        <?php if (!empty($c['plate'])): ?><span>· <?= hks_mob_h($c['plate']) ?></span><?php endif; ?>
    </div>
    <div class="mob-contact-footer">
        <?php if ($c['hks_registered']): ?>
            <span class="mob-hks-badge ok">HKS Kayıtlı</span>
        <?php else: ?>
            <span class="mob-hks-badge na">Doğrulanmamış</span>
        <?php endif; ?>
        <?php if (!empty($c['last_verified_at'])): ?>
        <span class="mob-contact-date"><?= hks_mob_h(date('d.m.Y', strtotime($c['last_verified_at']))) ?></span>
        <?php endif; ?>
    </div>
</div>
<?php endforeach; ?>
...
<div id="mobFormOverlay" class="mob-modal-overlay" onclick="if(event.target===this)mobCloseForm()">
    <div class="mob-modal-sheet" id="mobFormSheet">
        ...
        <div class="mob-modal-title">Müşteri Ekle</div>
